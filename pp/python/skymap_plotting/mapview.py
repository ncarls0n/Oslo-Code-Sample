...
if model=='tsz': vmin, vmax, cmap = -9.05 , -3.92 , cm.get_cmap('magma').copy()
if model=='ksz': vmin, vmax, cmap = -1.02e-5, 1.02e-5 , cm.get_cmap('coolwarm').copy()
if model=='kap': vmin, vmax, cmap = 0., 0.067, cm.get_cmap('jet').copy() 
...
if model=='cib': vmin, vmax, cmap = -5.75 , -1.25 , cm.get_cmap('hot').copy()
...
if (flatsky == 0 or flatsky==2):
    ...
    if cart:
        fov = 8.1821/2
        hp.cartview(map,fig=fig.number, xsize=figsize_inch[0]*dpi,
                cmap=cmap,cbar=None,title="", rot=(0,90,0),
                lonra=[-fov,fov],latra=[-fov,fov] )#, min=vmin,max=vmax   )
        #hp.cartview(map,fig=fig.number, xsize=figsize_inch[0]*dpi,cmap=cmap,cbar=None,title="", rot=(0,90,0), min=vmin,max=vmax, lonra=[-fov,fov],latra=[-fov,fov])

    #fig,ax = plt.subplots()
    #ax.imshow(hp_cart, origin='lower', extent=(fov,-fov,-fov,fov), interpolation='none')
...
if flatsky == 1:
    ...
    #Imshow map
    #im = ax.imshow(map, cmap=cmap,extent=[-fov/2,fov/2,-fov/2,fov/2],origin="upper")
    ...
    #Save figure
    fileout = 'Flatsky_'+str(model)+'_'+ntpath.basename(filein)[:-6]+'fov_'+str(fov)+'.png'
