"""
Adapted from Zack Li's script of the same name which was
...
Requires CAMB (pip install camb should work on CITA machines)
"""
...
def websky_pkcs_read(fname, num=0, offset=0):
    ...
    with open(fname, "r") as ifile:
        ...
        if num: n = num
        cat = np.fromfile(ifile, count=n*11, offset=offset*11*4, dtype=np.float32).reshape(n, 11)
    return cat
