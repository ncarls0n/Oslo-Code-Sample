...
class pykDict( dict ):
    ...
    def __getitem__( self, key ):
        if key not in self:
            if self.ask:
                print("pykDict: parameter '{0}' not found".format(key))
                val = ask_for( key )
                print("pykDict: setting '{0}' = {1}".format(key,repr(val)))
...
dict.read_from_file(sys.argv[1])

binFile=dict['binningFile']
taper = dict['taper']
gaussApod = dict['gaussianApodization']
applyMask = dict['applyMask']
hpfDict = dict['highPassCosSqFilter']
'''
