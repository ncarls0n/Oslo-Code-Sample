...
if len(sys.argv)>6:
    ...
    title1 = r'$\chi_E(\mathbf{q})$'
    title2 = r'$\zeta_G(\mathbf{q})$'
    title3 = r'$\zeta(\mathbf{q})$, $f_{nG}=5\times10^3$'
    title4 = r'$\bar{\rho}_G\delta_G(\mathbf{q})$'
    title5 = r'$\bar{\rho}\delta(\mathbf{q})$, $f_{nG}=5\times10^3$'
...
field1 = read_field_slice(fieldfile1,'xy',neff//2,nlattice,nbuff)
fig,axs[0] = slice_subplot(fig,axs[0],field1,X,Y,title1,r'$x$ [Mpc]',r'$y$ [Mpc]')
del(field1)

field2 = read_field_slice(fieldfile2,'xy',neff//2,nlattice,nbuff)
fig,axs[1] = slice_subplot(fig,axs[1],field2,X,Y,title2,r'$x$ [Mpc]',r'$y$ [Mpc]')

field3 = read_field_slice(fieldfile3,'xy',neff//2,nlattice,nbuff)
fig,axs[2] = slice_subplot(fig,axs[2],field2+field3,X,Y,title3,r'$x$ [Mpc]',r'$y$ [Mpc]')
del(field2,field3)

field4 = read_field_slice(fieldfile4,'xy',neff//2,nlattice,nbuff)
fig,axs[3] = slice_subplot(fig,axs[3],field4,X,Y,title4,r'$x$ [Mpc]',r'$y$ [Mpc]')
del(field4)

field5 = read_field_slice(fieldfile5,'xy',neff//2,nlattice,nbuff)
fig,axs[4] = slice_subplot(fig,axs[4],field5,X,Y,title5,r'$x$ [Mpc]',r'$y$ [Mpc]')
del(field5)
axs[1].axes.yaxis.set_ticklabels([])

#field3 = read_field_slice(fieldfile3,'xy',int(neff/2),nlattice,nbuff)
#fig,axs[2] = slice_subplot( fig, axs[2], field3, X,Y,title3,r'$x$ [Mpc]',r' ')
#del(field3)
#axs[2].axes.yaxis.set_ticklabels([])
#
#field4 = read_field_slice(fieldfile4,'xy',int(neff/2),nlattice,nbuff)
#fig,axs[3] = slice_subplot( fig, axs[3], field4, X,Y,title4,r'$x$ [Mpc]',r' ')
#del(field4)
