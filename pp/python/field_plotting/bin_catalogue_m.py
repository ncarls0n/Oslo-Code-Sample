...
ax.set_yscale('log')
ax.set_xscale('log')
ax.hist( RTH_m, bins=np.logspace(
    np.log10(m_halo_min*.5), np.log10(m_halo_max*1.05), 100 ))

ax.set_xlabel(r'halo mass $M_\mathrm{{halo}}$ $[M_{{\odot}}]$')
ax.set_ylabel(r'counts $n(M_\mathrm{{halo}})$')
ax.legend()
