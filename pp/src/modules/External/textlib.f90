...
CONTAINS 
  ...
  integer function i4arg(argno,defval)
    ...
    if( argno .le. command_argument_count() ) then
       call get_command_argument(argno,argi)
       if( onedot(argi) ) then
          i4arg = defval
       else
          i4arg = s2i4(argi)
...
function s256arg(argno, defval) result(result_string)
    implicit none
    ...
    character(len=256) :: argi
    integer :: status
    
    if (argno <= command_argument_count()) then
        call get_command_argument(argno, argi, status=status)
        if (status == 0) then
            if (onedot(argi)) then
                result_string = defval
            else
                result_string = trim(argi)
            end if
        else
            result_string = defval
        end if
  ...
  integer function s2i4(s)
    ...
    lens = indlnb(s) ! returns index last non-blank character in string s
    if( lens .gt. 99 ) then
