...
extern"C" void ReportMemory(const char *tag)
{
  ...
  // COMMENTED OUT TO ALLOW ASYNCHRONOUS MEMORY REPORTING
  //MPI_Allreduce(&vrt_local,&vrt,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
  //MPI_Allreduce(&ram_local,&ram,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
  ...
  vrt = vrt_local / pow(1024.,2);
  ram = ram_local / pow(1024.,2);

  if(myid==0){
    fprintf(stderr,"\n   %s:",tag);
    fprintf(stderr,"\n       virtual: %f GB for process %d",vrt,myid);
    fprintf(stderr,"\n      physical: %f GB for process %d",ram,myid);
    fprintf(stderr,"\n\n");
  }
