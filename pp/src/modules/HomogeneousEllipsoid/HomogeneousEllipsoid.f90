...
contains
  ...
  subroutine evolve_ellipse_full(idynax,zdynax,Ddynax,fcdynax)
    ...
    real delta_t, y10, y20, y30, lambda1_t, lambda2_t, lambda3_t, e_vt, p_vt, abar ! added for running homel on its own
    ...
    Ha_b_nr=sqrt((1.0+Rvac_nr*a_b**3+Rcurv_nr*a_b)/a_b) !=H(t)a(t) / (H_0 Omega_nr^.5 ) assuming Omega_r ~ 0
    Ha_b_nrinv=1.0/Ha_b_nr
    y(3)=a_b*(1.0-dlin*aLam_3)                 ! y(i) for i in {1,2,3} represent principle axis scale factors
    y(2)=a_b*(1.0-dlin*aLam_2)                 ! solving equations of motion (i.e. equation 2.21 in Bond & Myers
    y(1)=a_b*(1.0-dlin*aLam_1)                 ! 1997(I), note the similarity between y(1,2,3) and initial
    y(6)=Ha_b_nr*(1.0-(1.0+HD_Ha)*dlin*aLam_3) ! conditions equation 2.24). Note however that since dlin/=0, the
    y(5)=Ha_b_nr*(1.0-(1.0+HD_Ha)*dlin*aLam_2) ! resultant ellipsoid does not have eigenvalues Frho, e_v, p_v
    y(4)=Ha_b_nr*(1.0-(1.0+HD_Ha)*dlin*aLam_1) ! y(i) for i in {4,5,6} I'm guessing are first derivatives, used in solving for subsequent values of y(1), y(2), y(3), requires further investigation
    ...
    if(ihard==1.and.iwant_evmap==4) then
       ...
       y30=y(3)
       abar=1./(1./a_b+1)
       42 format( e14.8e2, 9(',',e14.8e2) )
       write(10,42) abar,y(1),y(2),y(3),Frho,e_v,p_v,aLam_1,aLam_2,aLam_3
    endif

111 continue ! continue to next line, begin evolving homogeneous ellipsoid
    istep=istep+1
    a_3=y(3)
    dtstep=tfac*a_3*sqrt(a_3)*Ha_b_nr ! 0.01 * (a_3)^(3/2) * 
    ...
    if(ihard==1.and.iwant_evmap==4) then
       abar   =1./(1./t+1.)
       delta_t=(Frho+1.)*y10*y20*y30*abar**3.0/( (1./(1./a_b+1))**3.0*y(1)*y(2)*y(3) ) - 1.
       p_vt   =(log(y(2))-.5*log(y(1))-.5*log(y(3))) / delta_t 
       e_vt   =(log(y(1))-log(y(3)))/(2.*delta_t)
       lambda1_t = delta_t*(1.+p_vt-3.*e_vt)/3.
       lambda2_t = delta_t*(1.-2.*p_vt)/3.
       lambda3_t = delta_t*(1.+p_vt+3.*e_vt)/3.
       write(10,42) abar,y(1),y(2),y(3),delta_t,e_vt,p_vt,lambda1_t,lambda2_t,lambda3_t
    endif
...
169       format('densvir,turn,cut: ',7(f9.3))
       elseif(ihard==1.and.iwant_evmap==4.and.lvirv(1)==1) then
          ! If last axis virialized, add a data point at z=0
          delta_t=(Frho+1)*y10*y20*y30/((1./(1./a_b+1))**3*y(1)*y(2)*y(3)) - 1
          write(10,42) 1.,y(1),y(2),y(3),delta_t,e_vt,p_vt,lambda1_t,lambda2_t,lambda3_t
          write(*,67) Frho,e_v,p_v,aLam_1,aLam_2,aLam_3
          write(*,66) (zdynv(ii),ii=1,nzdyn)
          write(*,68) zzc_3sph,zzc_Fsph,delta1_ev(3),delta1_ev(2),delta1_ev(1),delta1_ev(7)
          write(*,169) (delta1_ev(ii),ii=1,nzdyn)
          write(*,42) t,Frho, e_v, p_v 
       endif
       zdynax=zdynv(idynax)
       if(zdynax.eq.-1) then
          Ddynax=-1
  ...
  subroutine get_derivs(t,y,dy)
    ...
    a_b=t                ! or equation 2.23 of the first peak patch paper: Bond & Myers, 1996a aka BM1)
    a_b3=a_b**3
