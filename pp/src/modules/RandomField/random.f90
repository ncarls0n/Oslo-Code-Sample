...
contains
  ...
  subroutine gaussdev( Seed, GaussNum )
    ...
    ! Returns a Gaussian random deviate GaussNum with mu=0, sigma=1
 
    if (IGauss.eq.IZero) then !IGauss=0 on first call, IZero=0 is a parameter so always zero
       rsq=0.0d0
       do while (rsq.ge.1.0d0.or.rsq.le.0.0d0)
          call ranf(Seed,v1) !makes array of pseudorandom numbers v1(1:4) and changes seed numbers Seed(1:4)
          call ranf(Seed,v2) !makes second array of pseudorandom numbers v2(1:4) from changed seed 
          v1=2.0d0*v1-1.0d0
          v2=2.0d0*v2-1.0d0
          rsq=v1**2+v2**2
       end do
  ...
  subroutine ranfkbinary( K, KBinary )
    implicit none
    integer, dimension( IRandNumSize ) :: K!(1:4)
    integer, dimension( IBinarySize ) ::  KBinary!(1:48)

    integer, dimension( Mod4096DigitSize ) :: Bits!(1:12)
    integer X, I, J
    
    do I = 1, 4
       X = K( I ) / 2
       Bits( 1 ) = iranfodd( K( I ) ) !=mod( K(I),2 )
       do J = 2, Mod4096DigitSize 
          Bits( J ) = iranfodd( X ) !=mod(X,2)
          X = X / 2
       end do
