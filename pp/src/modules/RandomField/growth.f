...
CU    USES bsstep,hypdrv,hypser,odeint
      ...
      ! April 2024 edit - fixed a bug where odeint expects real y but
      ! gets complex y instead
      DO i = 1, 2
          y_real(i) = abs(y(i))
      END DO
      call odeint(y_real,4,0.,1.,EPS,.1,.0001,nok,nbad,hypdrv,bsstep)
      hypgeo=y_real(1)
      return
      END
