program RandomFieldDriver
  ...
  character *512 filepktab, outcode
  real           boxsize, ainit
  integer        nmesh, nbuff, ntile, seed, fieldid
  integer        NonGauss, fNL
  ...
  if(command_argument_count().lt.1) then
     if(myid==0) write(*,99)
99   format('usage: RandomFieldDriver <filepktab> <outcode> [<Lbox [Mpc]> <nmesh> <nbuff> <ntile> <ainit> <seed> <fieldflag> <nGflag> <fNL>]')
     call mpi_finalize(ierr)  
     stop 
  endif
     
  call get_command_argument(1,filepktab)
  call get_command_argument(2,outcode)
