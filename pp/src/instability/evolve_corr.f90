...
#include "peak-patch_macros.h"
  ...
  integer, parameter :: nstep     = __NSTEP__     ! maximum number of steps (default: 2**15)
  integer, parameter :: stepadapt = __STEPADAPT__ ! number of steps between adapting dt (default: 1)
  real(dl) :: dt0                 = __DT0__       ! base time step (defulat: 1._dl/dble(2**5)=0.03125_dl)
  real(dl) :: dt                                  ! adapted time step
  real(dl) :: t                                   ! time
  integer, parameter  :: nk   = __NK__            ! number of k modes
  real(dl), parameter :: kmax = __KMAX__          ! maximum k mode
  real(dl), parameter :: dk   = __DK__            ! fundamental k mode
  real(dl) :: k2                                  ! square wavenumber
  real(dl), parameter :: h_fac = 5._dl !4         ! number of efolds before horizon crossing to initialize a mode 
  ...
  real(dl), parameter :: phi_init = __PHI_INIT__, phi_fin = __PHI_FIN__
  ...
  do i=1,nk
     ...
     ! Report how many have been done
     if(mod(i-1,nk/5)==0) write(*,'(i4,a1,i4)') i,'/',nk
     !print*, i

     !k2 = dble(i**2)*dk**2 ! linear-spaced k
     k2 = (dk*(kmax/dk)**(dble(i)/dble(nk)))**2 ! log-spaced k
     ...
     !write(*,*) j ! to write out the number of iterations needed to converge
