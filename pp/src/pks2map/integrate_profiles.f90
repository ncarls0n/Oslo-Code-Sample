...
contains
  ...
  real function integrate_profile(theta,z,mh,halo_profile)
    ...
    ! pointers to functions in bbps_profile.f90
...
!    enddo
    if(halo_profile==1) then
        do i=1,n
            s = (i-0.5) * ds
            x = sqrt(s**2+b**2)
            y = y + bbps_ptilde(mh,z,x)
        enddo
    elseif(halo_profile==2) then
        do i=1,n
            s = (i-0.5) * ds
            x = sqrt(s**2+b**2)
            y = y + bbps_rhotilde(mh,z,x)
        enddo
    elseif(halo_profile==3) then
        do i=1,n
            s = (i-0.5) * ds
            x = sqrt(s**2+b**2)
            y = y + bbps_dmtilde(mh,z,x)
         enddo
     endif
