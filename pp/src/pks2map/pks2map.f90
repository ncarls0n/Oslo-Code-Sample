program pks2map
  ...
  ! Edit from April 2024: placed this label outside of all the conditional block
  ! just in case, and split the line so that it's not too long so that the 
  ! compiler is happy
11 format('Usage: pks2map <filein> <fileout> <tablefile> <profilecode> ' // &
         '[<zmin> <nside> <scramble> <center> <npix> <fov> <zmax> <chihview> ' // &
         '<model> <PSZcut>]  profile choices: tsz, ksz, kap, tau, tco, thi')
  ...
  ! Set field of view and resolution for flatsky square fov-by-fov degree maps
  fov   = fov / 360 * 2 * mypi ! convert angle to radians
  dpix  = fov / npix ! angular resolution for flatsky maps
...
13 format('Healpix Nside:      ',i0,/,&
  ...
  ! Create distance-redshift tables from src/pks2map/cosmology.f90
  call set_rofztable
  call set_zofrtable
  cut_low  = rofz(zmin)
  cut_high = rofz(zmax)
...
14 format('cut_low, cut_high:  ',f4.2,1x,f4.2,' Gpc')
  ...
  ! Cut all the halos not within redshift range
  m=0
  do i=1,nhalo 
     xh = posxyz(1,i)
     yh = posxyz(2,i)
     ...
     rth(m)      = rth(i)
  enddo
  nhalo = m ! reset nhalo to number of halos within redshift range
  ...
  ! Load profile table
  if (profile==itco)then
     call SFRtabload(tablefile)   ! load SFR Behroozi table from line_profile.f90
  else

     ! NOTE: This call to loadmaptable loads a table from peak-patch/tables that
     ! is set in the parameter file. The cuurent one only goes out to redshift
     ! 7, so we need to change this to a call to make_maptable that will make
     ! one for whatever size we need here!
     ! 
     ! - Nathan Carlson, January 2023

     call loadmaptable(tablefile) ! load BBPS table created from maptable.f90
  ...
  do i=1,nhalo ! Loop over halos and project each into map
     ...
     ! If PSZcut=1, halos below polynomial fit to Planck 2015 SZ selection function (see arXiv:1502.01598, fig. 26, 50% survey completeness contour). For mock ACT or SPT catalogues, set PSZcut=0
     ...
     ! Kernel functions for "field components" outside resolved halos (see equaitons 3.22-3.24 of Stein et al. 2020)
     mfh = 1.0 
...
18 format('Flatsky RMS:                ',1pe9.3,/,&
  ...
  open(unit=1, file=fileout_dp, form='unformatted', access='stream', status='replace')
  ...
  open(unit=1, file=fileout_fs, form='unformatted', access='stream', status='replace')
