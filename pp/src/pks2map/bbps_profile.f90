...
contains
  ...
  real function bbps_ptilde(mh,z,x)
    ...
    ! see Battaglia et al. 2012, ApJ, 758, 75 [arXiv:1109.3711] a.k.a.
    ! "BBPS" section 4.1 for precise definitions of P_Delta and Ptilde
    ! ----------------------------------------------------------------
    ...
    ! P0 (see BBPS eq. 11 & table 1)
    A0     = P0_Am(i)
    alpham = P0_alpham(i)
    alphaz = P0_alphaz(i)
    P0     = A0*(mh/1e14)**alpham*(1+z)**alphaz

    ! xc (see BBPS eq. 11 & table 1)
    A0     = xc_Am(i)
    alpham = xc_alpham(i)
    alphaz = xc_alphaz(i)
    xc     = A0*(mh/1e14)**alpham*(1+z)**alphaz

    ! beta (see BBPS eq. 11 & table 1)
    A0     = beta_Am(i)
...
!    bbps_dmtilde  = P0*(x/xc_dm)**(-1.0*gamma_dm)*(1+(x/xc_dm)**alpha_dm)**(-1.0*(beta-gamma_dm)/alpha_dm)
  ...
  real function nfw_dmtilde(mh,z,x)
    ...
    ! Uses concentration parameter from Duffy 2008 [0804.2486]
  ...
  real function C_duffy(mh,z)
    ...
    ! ----------------------------------------------------------------
    ! Concentration parameter from Duffy 2008 [0804.2486]
    ! ----------------------------------------------------------------
