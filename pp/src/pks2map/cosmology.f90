module cosmology
  ...
  parameter (sigmat = 6.65246d-25  ,& ! Thompson cross-section for electron (cm^2)
                  c = 2.99792458d10,&
                 me = 9.109383d-28 ,&
                mue = 1.136        ,&
                 mp = 1.67e-24     ,&
  ...
  ! e.g. see normalization coefficient in equation 3.12 of [2001.08787],
  ! 1.932 is (8-5*Y_p)/(4-2*Y_p) whwere Y_p=0.24 is the helium abundance


  double precision tau0persigma ! this is the quantity 
                                ! sigmat/mu/mp*rho100^(2/3) * (3/4*pi*msun)**(1./3)
                                ! in units of Msun^(-1/3)
  parameter (tau0persigma = sigmat/mue/mp * (3./4/pi*msun)**(1./3) * rho100**(2./3))
...
contains
  ...
  real function deltacrit(z)
    ...
    ! density; see Bryan & Norman (1997) equation 6 [astro-ph:9710107]
