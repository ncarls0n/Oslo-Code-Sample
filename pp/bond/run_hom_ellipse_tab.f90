...
subroutine run_hom_ellipse_tab
  ...
  parameter (ny=6)
  common/wantevpvtab/iwant_evpvtab
  common/evpvtable/Nfsc,ifc1inv,Nev,Npv_ev,devtab,dpv_evtab,evtabmax,pv_evtabmax,Fsmin,Fsmax,zinit_fac_ell,& 
       fsctabv(Nfscmax),Ftabv(Nfscmax),fc1mat(Npv_evmin:Npv_evmax,0:Nevmax,Nfscmax)
  dimension quicktab(0:1,0:1)
  common/params/iwant_evmap,tfac,zinit,nstepmax,nyp,nout,dcrit,Fbar,e_vmax,de_v,p_vbar,dp_v,e_vbar
  common/equil/fcoll_3,fcoll_2,fcoll_1,a_3eq,a_2eq,a_1eq,lvirv(3),ivir_strat,iforce_strat
  common/evalues/aLam_1,aLam_2,aLam_3,Frho,e_v,p_v
  common/hard/ihard
  common/rd/iwant_rd
...
1915 continue
  ...
  if(iwant_evmap.ne.0) then
     if(iwant_evpvtab.eq.1) then
        ...
        read(*,*) Nfsc,ifc1inv,Fsmin,Fsmax,zinit_fac_ell,devtab,dpv_evtab,evtabmax,pv_evtabmax
        if(Nfsc.gt.1) then
