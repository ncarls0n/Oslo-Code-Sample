...
#                                                                         #
# USAGE:                                                                  #
#     python3 <...>/peak-patch/tools/get_websky_Cl.py <map1>.fits         #
#         <label1> <map2>.fits <label2> ...                               #
#                                                                         #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
if len(sys.argv)%2 != 1:
    raise SyntaxError('You\'ve passed the wrong number of arguments at com'
        ...
        +'get_websky_Cl.py <map1>.fits <label1>\n        <map2>.fits <labe'
        +'l2> ...')
...
for i in range(1,len(sys.argv),2):
    ...
    if i==1:
        ...
        if   'tsz' in filein.lower(): model='tSZ'
            ...
            raise Warning('type of map passed at command line not recogniz'
    ...
    # Plot websky C_l power spectrum
    ax.plot( l, D_l, label=r'WebSky {0}'.format(label) )
    del(skymap,C_l,l,D_l) # Free up some RAM
...
if model=='tSZ':
    ...
    act_ebar = ax.errorbar( 3000, 1e12*D_act, yerr=1e12*dD_act, ls='none',
        marker='s', ms=8, mew=.5, mec='w', mfc='navy',
        elinewidth=2, ecolor='navy', label=r'ACT 2013' ,
        zorder=int((len(sys.argv)-1)/2)+3 )
    ...
    spt_ebar = ax.errorbar( 3000, 1e12*D_spt,
        ...
        ls='none', marker='X', ms=8, mew=.5, mec='w', mfc='teal',
        elinewidth=1, ecolor='teal', label=r'SPT 2015' ,
        zorder=int((len(sys.argv)-1)/2)+4 )
