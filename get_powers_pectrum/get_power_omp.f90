program get_powerspectrum
    ...
    character(len=1024) field_file, out_file
    ...
    integer(c_size_t) fft_size
    integer(c_int)    nfft, nthreads
    ...
    ! Allocate parallel FFTW arrays and setup pointers to C arrays
    nfft     = n
    ...
    ! Initialize parallel FFTW plans for single-precision 3D discrete Fourier transforms
    call cpu_time(start)
    ...
    write(*,*)
    write(*,'(a25,es10.4e2,a8)') 'time to make FFT plans: ', finish-start, ' seconds'
    ...
    close(99)
    call cpu_time(finish)
    write(*,'(a24,es10.4e2,a8)') 'time to read 3D field: ', finish-start, ' seconds'

    call cpu_time(start)
    ! Calculate 3D DFT of field F(f_x)=f_k
    call fftwf_execute_dft_r2c(FT_plan, f_x, f_k)
    call cpu_time(finish)
    write(*,'(a22,es10.4e2,a8)') 'time to execute FFT: ', finish-start, ' seconds'
    ...
    call cpu_time(start)
!$OMP PARALLEL DO PRIVATE(ii,jj,kk,mode,l,weights) FIRSTPRIVATE(n,nyquist) REDUCTION(+:P_k,P_0)
...
!$OMP END PARALLEL DO
    call cpu_time(finish)
    write(*,'(a35,es10.4e2,a8)') 'time to calculate power spectrum: ', &
                                 (finish-start)/float(nthreads), ' seconds'

    ! Divides out zero mode
    do i=1,klen
    ...
    P_k = (s_box/(2.*pi*float(n*(n-2*nbuff))))**3. * P_k

    ! Write out fundamental mode
    write(*,*)
    write(*,'(a44,es10.4e2,a8)') 'The 0th Fourier mode is P(k=0)=<|f(k=0)|^2>=',P_k(1),' Mpc^-1.'
    write(*,*)

    ! Save power spectrum in the form used by Peak Patch initial conditions
    out_file = trim(field_file)//'_power.dat'
    open(unit=42,file=out_file)
    do i=2,klen
        write(42,'(e10.5e2,a,e10.5e2)') kbins(i),' ',P_k(i)
    enddo
    close(42)
    write(*,'(a23)') 'P(k>0) written to file:'
    write(*,*) trim(out_file)
    write(*,*)
